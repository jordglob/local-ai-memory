# =============================================================================
#  aimem_ingest.py — the python engine behind ai-memory-ingest.sh.
#  Run via the bash entrypoint (the public interface), never imported by it:
#  `bash ai-memory-ingest.sh …` execs `python3 <libdir>/aimem_ingest.py …`.
#  Shares scrub/slug/atomic-write helpers with the search engine via
#  aimem_common (same directory). Python 3.8+ stdlib only.
# =============================================================================
...
# ── secret-scrub ──────────────────────────────────────────────────────────────
# scrub_secrets (+ its pattern set) lives in aimem_common — every import calls
# it before anything is written, so a synced/exported vault upholds "secrets
# never travel" even when the original chat contained a pasted key.
...
# ── shared output (slugify + _atomic_write come from aimem_common) ───────────
def _safe_id(s, n=40):
...
def main():
    ...
    if pos and (pos[-1].endswith(".zip") or Path(pos[-1]).expanduser().is_file()):
        zip_arg = pos.pop()
    vault = Path(pos[0]).expanduser() if pos else default_vault()
