...
#
#  This is an OPTIONAL, isolated surface (like remote.sh): it never touches the
#  vault's contents, never stores or prints a secret, and only manages a tmux
#  session that YOU start. It applies its tmux options at the SESSION scope, so
#  it does not clobber your ~/.tmux.conf (use `install-tmux-conf` to opt into a
